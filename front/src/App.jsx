...
class App extends React.Component {
  ...
  lookupTaskByItsTitle(taskList, taskText) {
    console.log(taskList);
    const neededTaskList = taskList.find((l) => {   
    ...
    return neededTaskList
  }

  changeTaskState(neededTaskList, taskText, isArchive = false) {
    console.log(neededTaskList);
    ...
    if (neededTaskList !== undefined) {
      ...
      fetch(url, {
        ...
        .then(json => {
          ...
          if (!isArchive) {
            ...
            this.state.assistant.sendData({action: {action_id: "taskCompletionSuccess", parameters: {}}})
          } else {
            this.state.assistant.sendData({action: {action_id: "taskReturnSuccess", parameters: {}}})
            this.updateTaskList("Активные")
          }
        })
      ...
      if (!isArchive)
        this.state.assistant.sendData({action: {action_id: "taskCompletionFail", parameters: {}}})
      else 
        this.state.assistant.sendData({action: {action_id: "taskReturnFail", parameters: {}}})
    }
      
  }

  prepTaskStatusChange(taskText, lookArchived) {
    
    if (lookArchived) {
      const isArchive = true;
      let getUrlSuffix = `sber_user_id=${this.state.sberUserId}`;
      const url = `${BASE_URL}/api/formated_tasks/?isCompleted=True&${getUrlSuffix}`;
      fetch(url)
      .then(response => response.json())
      .then(json => this.changeTaskState(this.lookupTaskByItsTitle(json.tasks, taskText), taskText, isArchive))
      .catch(e => console.log(e)); 

    } else if (this.state.tasks.length > 0) {
      this.changeTaskState(this.lookupTaskByItsTitle(this.state.tasks, taskText), taskText);
  ...
  assistantCompleteTask(action) {
    if (action) {
      this.prepTaskStatusChange(action.task_title, false)
    }
  }

  assistantReturnTask(action) {
    if (action) {
      this.prepTaskStatusChange(action.task_title, true)
    }
  }
  ...
  dispatchAssistantAction(action) {
    if (action && action.type) {
      switch (action.type) {
        ...
        case "return_task":
          this.assistantReturnTask(action);
          break;
      ...
      <div className='app-wrapper'>
        ...
        <div className='app-content-wrapper'>
          <Navbar
            ...
            setModalContent={value => this.setState({ modalContent: value})}
          />
          <Tasks
